...
class User(Base):
    ...
    id = Column(PgUUID(as_uuid=True), primary_key=True, default=uuid_mod.uuid4, index=True)
    ...
    preferred_language = Column(
        Enum(SupportedLanguage, values_callable=lambda obj: [e.value for e in obj], native_enum=False),
    ...
    two_fa_backup_codes = Column(Text, nullable=True)  # JSON array of hashed backup codes
    ...
    template_favorites = relationship("TemplateFavorite", back_populates="user", cascade="all, delete-orphan")
    reference_documents = relationship("ReferenceDocument", back_populates="uploaded_by_user")
    subscription = relationship("Subscription", back_populates="user", uselist=False)
    approved_by_user = relationship("User", remote_side="User.id", foreign_keys=[approved_by])
