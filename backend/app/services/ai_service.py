...
async def _fetch_training_context(
    assessment_data: Dict[str, Any], user_id: str = None
) -> str:
    ...
    try:
        ...
        all_results = []
        for q in queries:
            # Search user uploads first, then all RAG knowledge base datasets
            results = await unified_rag.search(
                query=q,
                user_id=user_id,
                top_k=5,
                min_score=0.3,
                source_tiers=None,  # Search ALL tiers (user + all 11 RAG datasets)
            )
...
7. extracted_fields (object with any fields you can extract from the OCR text or description):
   - inception_date (ISO date string YYYY-MM-DD if found)
   - expiry_date (ISO date string YYYY-MM-DD if found)
   - insured_entity_name (full legal entity name if found)
   - broker_name (broker/intermediary name if found)
   - territory (country/region if found)
...
class AIService:
        ...
        if self._initialized:
            try:
                ai_result = await self._run_ai_analysis(
                    assessment_data, training_context
                )
                if ai_result:
                    return ai_result
            except Exception as e:
                ...
                logger.error(
                    f"AI analysis failed, falling back to rules: {e}\n{traceback.format_exc()}"
                )
    ...
    def _prepare_assessment_data(self, assessment) -> Dict[str, Any]:
        ...
        return {
            ...
            "deductible": float(assessment.deductible)
            if assessment.deductible
            else None,
            ...
            "ocr_extracted_text": (assessment.ocr_extracted_text or "")[
                :2000
            ],  # Limit context
        }

    async def _run_ai_analysis(
        self, assessment_data: Dict[str, Any], training_context: str = ""
    ) -> Dict[str, Any]:
...
Return JSON with: risk_score, confidence_score, risk_factors, recommendations, decision, summary"""
        ...
        try:
            ...
            try:
                ...
                return {
                    ...
                    "suggested_decision": "GO"
                    if result.get("confidence_score", 50) >= 60
                    else "NO-GO",
                    "ai_powered": True,
    ...
    def _identify_risk_factors(self, data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if data.get("territory") and any(
            t in str(data["territory"]) for t in high_risk_territories
        ):
            risk_factors.append(
                {
        ...
        if (
            ...
            and data["risk_category"].lower() in high_risk_categories
        ):
            risk_factors.append(
                {
    ...
    def _generate_recommendations(
        self, risk_factors: List[Dict], data: Dict
    ) -> List[str]:
        ...
        for factor in risk_factors:
            if factor["severity"] == "high":
                recommendations.append(
                    f"CRITICAL: Address {factor['factor']} - {factor['description']}"
                )
            elif factor["severity"] == "medium":
                recommendations.append(
                    f"Review {factor['factor']}: Consider additional terms or exclusions"
                )

        if data.get("sum_insured") and data["sum_insured"] > 50_000_000:
            recommendations.append(
                "Consider requiring engineering survey or loss control report"
            )
    ...
    def _generate_summary(self, data: Dict, risk_score: int) -> str:
        ...
        risk_level = (
            "low" if risk_score < 40 else "moderate" if risk_score < 70 else "high"
        )
    ...
    def _analyze_pricing(self, data: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendation": "Consider rate increase"
            if adequacy != "adequate"
            else "Rate appears adequate",
        }
    ...
    def _analyze_exposure(self, data: Dict) -> Dict[str, Any]:
        ...
        return {
            ...
            "deductible_ratio": round(data["deductible"] / data["sum_insured"] * 100, 2)
            if data.get("deductible") and data.get("sum_insured")
            else None,
            "concentration": "high"
            if (data.get("sum_insured") or 0) > 50_000_000
            else "normal",
            "details": exposure,
        }
    ...
    def _analyze_territory(self, data: Dict) -> Dict[str, Any]:
        ...
        risk_level = (
            "high"
            if any(t in str(territory) for t in high_risk)
            else "medium"
            if any(t in str(territory) for t in medium_risk)
            else "standard"
        )
    ...
    def _get_territory_considerations(
        self, territory: str, risk_level: str
    ) -> List[str]:
