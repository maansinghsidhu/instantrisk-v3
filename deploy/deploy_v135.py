"""
Deploy v135: Fix document generation pipeline — clause resolution + unified flows.

Changes vs v134:
- document_generation.py: INLINE_MANDATORY_CLAUSES dict (20 clauses w/ full text);
  _resolve_clause_full_text() + _resolve_clauses_batch() helpers;
  _run_generation_job() resolves clause IDs to full text before opendraft;
  generate_documents_opendraft() uses _resolve_clauses_batch() (not 200-char previews);
  suggest_documents() uses inline fallback when library empty + text_preview in all suggestions;
  ai_clause_search() returns full_text + content_preview, uses inline fallback for mandatory clauses
- generated_document.py: text_preview: Optional[str] = None field added to LMAClauseSuggestion
- opendraft_generator.py: stronger clause-to-section matching by title keywords;
  minimum content threshold (<100 chars => auto-enrich from defaults)
- document_configure_screen.dart: text_preview displayed under clause names in clause cards
...
print("Fix document generation pipeline — clause resolution + unified flows")
print("=" * 70)
