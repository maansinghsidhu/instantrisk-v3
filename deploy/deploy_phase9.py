"""Deploy Phase 9: Full AutoGen + Chat Integration + Bedrock Optimization.

Changes included:
- Gate 1: Chat router already registered, tool format fixed
- Gate 2: AutoGen processor already exists in backend-merged
- Gate 3: Haiku/Sonnet model selection per agent (AGENT_MODEL_MAP)
- Gate 4: Context summarization + Redis caching for ClaimSense
- Gate 5: ClaimSense + RapidRate tool integration in pipeline

To run:
1. Get fresh AWS credentials from SSO portal
2. Replace REPLACE_ME placeholders below
3. Run: python deploy_phase9.py
"""
import os, zipfile, boto3, json, time, sys, urllib.request, urllib.error

# PASTE FRESH AWS CREDENTIALS HERE (from SSO portal)
    ...
    try:
        ...
        if primary:
            ...
            if p.get('runningCount', 0) >= 1 and p.get('rolloutState') == 'COMPLETED':
                ...
                print('\n=== Phase 9 Features ===')
                print('- Haiku/Sonnet model selection per agent')
                print('- Context summarization for large documents')
                print('- Redis caching for ClaimSense benchmarks (24h TTL)')
                print('- ClaimSense benchmark integration in RiskAnalyst')
                print('- RapidRate pricing integration in Underwriter')
                sys.exit(0)
