"""Deploy v23: Haiku model selection per agent (Gate 3).

Changes:
- Updated _run_agent() to accept agent_name parameter
- Added AGENT_MODEL_MAP: Haiku for simple agents, Sonnet for reasoning agents
- All 14 agent calls now pass agent_name for model selection
"""
import os, zipfile, boto3, json, time, sys, urllib.request, urllib.error

# PASTE FRESH AWS CREDENTIALS HERE (from SSO portal)
# AWS_ACCESS_KEY_ID - set via environment before running
# AWS_SECRET_ACCESS_KEY - set via environment before running
# AWS_SESSION_TOKEN - set via environment before running
